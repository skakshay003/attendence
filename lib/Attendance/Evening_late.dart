...
class Evening_Late_Page extends StatefulWidget {
  const Evening_Late_Page({super.key});

  @override
  State<Evening_Late_Page> createState() => _Evening_Late_PageState();
}

class _Evening_Late_PageState extends State<Evening_Late_Page> {
...
// Iterate over fetchdata to calculate late count for each user_id
            fetchdata.forEach((entry) {
              ...
              String workto = entry['work_to'];
              String clkOutTime = entry['clk_out_tm'];

              // Parse time strings to DateTime objects, handling possible FormatException
              try {
                DateTime workToTime = DateTime.parse('2000-01-01 $workto');
                DateTime clkOutTimeParsed =
                    DateTime.parse('2000-01-01 $clkOutTime');

                // Check if clk_in_tm is greater than work_frm
                if (clkOutTimeParsed.isBefore(workToTime)) {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        filteredData = att_det.where((entry) {
          String clkOutTime = entry['clk_out_tm'];
          String workToTime = entry['work_to'];

          try {
            DateTime clkOutTimeParsed = DateTime.parse('2000-01-01 $clkOutTime');
            DateTime workToTimeParsed =
                DateTime.parse('2000-01-01 $workToTime');

            // Check if clk_in_tm is greater than work_frm
            return clkOutTimeParsed.isBefore(workToTimeParsed);
          } catch (e) {
            print('Error parsing date: $e');
                ...
                child: DataTable(
                  ...
                  rows: filteredData.map<DataRow>((entry) {
                    ...
                    DateTime formattedTime = DateTime.parse(
                        '2000-01-01 ${entry['clk_out_tm'].toString()}');
                    ...
                    DateTime workToTime =
                        DateFormat('HH:mm:ss').parse(entry['work_to']);
                    DateTime clkOutTime =
                        DateFormat('HH:mm:ss').parse(entry['clk_out_tm']);

                    // Ensure that the clkInTime is on the same day as workFromTime
                    if (clkOutTime.isBefore(workToTime)) {
                      clkOutTime = clkOutTime.add(Duration(days: 0));
                    }

                    // Calculate the difference
                    Duration difference = workToTime.difference(clkOutTime);
