...
class View_Employee extends StatefulWidget {
  const View_Employee({Key? key});

  @override
  State<View_Employee> createState() => _View_State();
}

class _View_State extends State<View_Employee> {
  ...
  Future<void> viewemployee() async {
    try {
      var apiUrl = Uri.parse(
          '$backendIP/view/vfetchemployee.php'); // Replace with the URL of your PHP script
      ...
      if (response.statusCode == 200) {
        ...
        if (data is Map<String, dynamic> && data.containsKey('message')) {
          if (data['message'] == 'Data updated successfully') {
            ...
            await viewemployee(); // Refresh the data after update
          } else {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Center(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['message'] == 'Department deleted successfully') {
          ...
          await viewemployee(); // Refresh the data after deletion
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
