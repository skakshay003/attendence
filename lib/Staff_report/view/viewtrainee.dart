...
class View_Trainee extends StatefulWidget {
  const View_Trainee({Key? key});

  @override
  State<View_Trainee> createState() => _View_State();
}

class _View_State extends State<View_Trainee> {
  ...
  Future<void> viewtrainee() async {
    try {
      var apiUrl = Uri.parse(
          '$backendIP/view/vfetchtrianee.php'); // Replace with the URL of your PHP script
  ...
  Future<void> showEditDialog(BuildContext context, Map<String, dynamic> data,
    ...
    TextEditingController locatController =
        TextEditingController(text: (data['locca']=='')?'Select Location':data['locca']);
    ...
    TextEditingController bloodController =
        TextEditingController(text: (data['blood']=='')?'Select Blood Group':data['blood']);
...
void showSnack(String text) {
  if (_scaffoldKey.currentContext != null) {
    ScaffoldMessenger.of(_scaffoldKey.currentContext!)
        .showSnackBar(SnackBar(content: Center(child: Text(text,style: TextStyle(fontWeight: FontWeight.bold,color: Colors.black),)),
  ...
  if (picked != null) {
    ...
    final minimumBirthDate = DateTime.now().subtract(Duration(days: 18 * 365));
      ...
      if (response.statusCode == 200) {
        ...
        if (data is Map<String, dynamic> && data.containsKey('message')) {
          if (data['message'] == 'Data updated successfully') {
            ...
            await viewtrainee(); // Refresh the data after update
          } else {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Center(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['message'] == 'Department deleted successfully') {
          ...
          await viewtrainee(); // Refresh the data after deletion
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
